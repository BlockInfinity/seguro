...
func initialModel() model {
	return model{
		...
		choices: []string{"Buy carrots\ntt\nabc", "Buy celery", "Buy kohlrabi"},
...
func (m model) View() string {
	// The header
	question := "What should we buy at the market?\n\n"

	// Iterate over our choices
	for choiceNumber, choice := range m.choices {
		// Is the cursor pointing at this choice?
		cursor := " " // no cursor
		if m.cursor == choiceNumber {
			cursor = ">" // cursor!
		}

		// Is this choice selected?
		checked := " " // not selected
		if _, ok := m.selected[choiceNumber]; ok {
			checked = "x" // selected!
		}
...
func commandFix(gitMode bool, disabledDetectors []string,
	...
	fmt.Println(unifiedFindingsNotIgnored)
